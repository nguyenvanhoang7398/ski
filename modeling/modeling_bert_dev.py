    ...
    def __init__(self, config, entity_structure):
        ...
        if entity_structure != 'none':
            ...
            if entity_structure == 'decomp':
                ...
                self.bili_0 = nn.Parameter(nn.init.xavier_uniform_(torch.empty(
                    self.num_attention_heads, self.attention_head_size, self.attention_head_size)))
                self.bili_1 = nn.Parameter(nn.init.xavier_uniform_(torch.empty(
                    self.num_attention_heads, self.attention_head_size, self.attention_head_size)))
                self.bili_2 = nn.Parameter(nn.init.xavier_uniform_(torch.empty(
                    self.num_attention_heads, self.attention_head_size, self.attention_head_size)))
                self.bili_3 = nn.Parameter(nn.init.xavier_uniform_(torch.empty(
                    self.num_attention_heads, self.attention_head_size, self.attention_head_size)))
                self.bili_4 = nn.Parameter(nn.init.xavier_uniform_(torch.empty(
                    self.num_attention_heads, self.attention_head_size, self.attention_head_size)))

            self.abs_bias_0 = nn.Parameter(torch.zeros(self.num_attention_heads))
            self.abs_bias_1 = nn.Parameter(torch.zeros(self.num_attention_heads))
            self.abs_bias_2 = nn.Parameter(torch.zeros(self.num_attention_heads))
            self.abs_bias_3 = nn.Parameter(torch.zeros(self.num_attention_heads))
            self.abs_bias_4 = nn.Parameter(torch.zeros(self.num_attention_heads))
        ...
        if self.entity_structure != 'none':
            for i in range(5):
                if self.entity_structure == 'decomp':
                    ...
                    if i == 0:
                        attention_bias = torch.einsum("bnip,npq,bnjq->bnij", query_layer, self.bili_0, key_layer)
                        attention_scores += (attention_bias + self.abs_bias_0[None, :, None, None]) * structure_mask[i]
                    elif i == 1:
                        attention_bias = torch.einsum("bnip,npq,bnjq->bnij", query_layer, self.bili_1, key_layer)
                        attention_scores += (attention_bias + self.abs_bias_1[None, :, None, None]) * structure_mask[i]
                    elif i == 2:
                        attention_bias = torch.einsum("bnip,npq,bnjq->bnij", query_layer, self.bili_2, key_layer)
                        attention_scores += (attention_bias + self.abs_bias_2[None, :, None, None]) * structure_mask[i]
                    elif i == 3:
                        attention_bias = torch.einsum("bnip,npq,bnjq->bnij", query_layer, self.bili_3, key_layer)
                        attention_scores += (attention_bias + self.abs_bias_3[None, :, None, None]) * structure_mask[i]
                    else:
                        attention_bias = torch.einsum("bnip,npq,bnjq->bnij", query_layer, self.bili_4, key_layer)
                        attention_scores += (attention_bias + self.abs_bias_4[None, :, None, None]) * structure_mask[i]
...
class BertLayer(nn.Module):
    def __init__(self, config, entity_structure):
        ...
        if self.is_decoder:
            self.crossattention = BertAttention(config, entity_structure)
...
class BertModel(BertPreTrainedModel):
    ...
    Examples::
        ...
        # DEBUG: hard code the dtype to float32 to be compatible with parallel without parameter list
