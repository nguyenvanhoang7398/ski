...
from model import (BertForDocRED, RobertaForDocRED, TrentForDocRED)
                ...
                if args.local_rank in [-1, 0] and args.logging_steps > 0 and global_step % args.logging_steps == 0:
                    ...
                    if (
                        ...
                        for key, value in results.items():
                            ...
                            if key == main_key and value > best_value:
                                best_value = value
                                print("Save best model checkpoint to", args.output_dir)
                                model_to_save = (
                    ...
                    writer.add_scalars("logs", logs, global_step)
...
def evaluate(args, model, tokenizer, prefix=""):
    ...
    total_labels = 0
    output_preds = []
    for (i, (example, pred, ent_mask)) in enumerate(tqdm(zip(dev_examples, preds, ent_masks), desc="Post-process eval")):
    ...
    pr_y = []
    correct = 0
    for i, pred in enumerate(output_preds):     # determine the best threshold to include the prediction of relation for best f1
        correct += pred[0]
    ...
    utils.save_json(result, "dev_results.json")
...
def predict(args, model, tokenizer, prefix=""):
    processor = DocREDProcessor()
    pred_examples = processor.get_dev_examples(args.data_dir)
    ...
    for batch in tqdm(eval_dataloader, desc="Evaluating"):
        ...
        with torch.no_grad():
            ...
            stacked = torch.stack(attentions, dim=1)
            reduced_1 = torch.sum(stacked, dim=1)
            averaged_attentions = torch.sum(reduced_1, dim=1)
            all_attentions.append(averaged_attentions.detach().cpu().numpy())
    ...
    utils.save_to_pickle(all_attentions, output_attention_file)
...
def main():
    ...
    elif args.model_type == 'trent':
        ModelArch = TrentForDocRED
