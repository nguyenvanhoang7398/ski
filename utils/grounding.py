...
def ground_docred_examples(example):
    global nlp, entities
    if nlp is None:
        nlp = spacy.load('en_core_web_sm')
        nlp.add_pipe("entityLinker", last=True)

    tokenized_sents = example["sents"]
    doc_sents = [" ".join(s) for s in tokenized_sents]
    doc_text = " ".join(doc_sents)
    return ground_doc_text(nlp, doc_text, example)


def ground_doc_text(nlp, doc_text, example):
    try:
        doc = nlp(doc_text)
        all_linked_entities = doc._.linkedEntities.entities
    except Exception as e:
        print(e)
        return example
    # iterates over sentences and prints linked entities

    sent_size = [sum([len(x) for x in y]) + len(y) - (1 if i == len(example["sents"])-1 else 0) for i, y in enumerate(example["sents"])]
    all_sent_start_pos = [sum(sent_size[:i]) for i in range(len(sent_size))]

    for docred_entity in example["vertexSet"]:
        # first loop through each docred entity in the example
        # we'll try to link each docred entity with a wiki entity
        linked_entity, best_match_ratio = None, 0.
        for mention in docred_entity:
            # second loop through each mention of the entity
            for wiki_entity in all_linked_entities:
                # try to find the sentence id of the wiki entity
                start_char, end_char = wiki_entity.span.start_char, wiki_entity.span.end_char
                wiki_entity_sent_id = len(all_sent_start_pos)
                for i, sent_start in enumerate(all_sent_start_pos):
                    if i > 0 and start_char < sent_start:
                        wiki_entity_sent_id = i-1
                        break
                if mention["sent_id"] == wiki_entity_sent_id:
                    # try to find the start pos of the sent
                    start_sent_pos = 0 if wiki_entity_sent_id == 0 else all_sent_start_pos[wiki_entity_sent_id]

                    ws, we = start_char - start_sent_pos, end_char - start_sent_pos
                    start_pos, end_pos = mention["pos"][0], mention["pos"][1]
                    ds_prev = example["sents"][mention["sent_id"]][:start_pos]
                    ds = sum([len(_) for _ in ds_prev]) + len(ds_prev)
                    de_prev = example["sents"][mention["sent_id"]][:end_pos]
                    de = sum([len(_) for _ in de_prev]) + len(de_prev) - 1

                    if ws >= de or ds >= we:
                        match_ratio = 0
                    elif ws >= ds and we <= de:
                        # wiki in docred
                        match_ratio = float(we - ws) / float(de - ds)
                    elif ds >= ws and de <= we:
                        match_ratio = float(de - ds) / float(we - ws)
                    elif ds < ws < de < we:
                        # docred first
                        match_ratio = float(de - ws) / float(we - ds)
                    else:
                        match_ratio = float(we - ds) / float(de - ws)

                    if match_ratio > best_match_ratio and wiki_entity.original_alias in entities:
                        linked_entity = wiki_entity
                        best_match_ratio = match_ratio
        for mention in docred_entity:
            if linked_entity is not None:
                mention["ent_alias"] = linked_entity.original_alias
                mention["ent_identifier"] = "Q" + str(linked_entity.identifier)
            pass
    return example


def ski_ground(docred_split_path, wiki_entity_path, output_path, num_processes):
    docred_data = load_json(docred_split_path)
    global entities
    entities = load_text_as_list(wiki_entity_path)

    if num_processes > 1:
        with Pool(num_processes) as p:
            res = list(tqdm(p.imap(ground_docred_examples, docred_data), total=len(docred_data)))
    else:
        res = []
        for example in tqdm(docred_data, desc="Grounding"):
            r = ground_docred_examples(example)
            res.append(r)
    save_json(res, output_path)
    print(f'grounded entities saved to {output_path}')
    print()
